...
## 1.0.0 (2026-08-02) - Initial fork and complete product rebrand
...
- Renamed to RevivalPad throughout: executable, window titles, tray, About
  dialog, CLI help, notifications, log output and error messages.
- All identity values (name, version, application id, URLs, config paths)
  centralised in the CMake identity block and exposed through a generated
  `config.h`, instead of being repeated as literals across the sources.
- New icon set drawn for RevivalPad. No AntiMicroX artwork is reused.

**Configuration and compatibility:**

- RevivalPad uses its own configuration namespace
  (`revivalpad/revivalpad_settings.ini`) and its own local socket key, so it can
  be installed and run alongside AntiMicroX.
- First-run migration copies an existing AntiMicroX, `antimicroX` or `antimicro`
  configuration into the RevivalPad location. Originals are never moved or
  deleted; an existing RevivalPad configuration is never overwritten.
- The `.amgp` profile format and `application/x-amgp` MIME type are unchanged.
  Existing profiles open without conversion.

**Breaking integration changes:**

- **D-Bus:** the service and interface are now
  `io.github._0x1_1.revivalpad[.InputDevice]`, not `io.github.antimicrox`.
  Object paths (`/InputDevice/<n>`) are unchanged. Scripts driving AntiMicroX
  over D-Bus must be updated.
- **Windows file association:** the `.amgp` ProgID changed from `AntiMicro.amgp`
  to `RevivalPad.amgp`. The file extension is unchanged.

**Other:**

- Update checker targets the RevivalPad repository; it never queries the
  AntiMicroX release endpoint.
- Linux application id, desktop entry, AppStream component, MIME package, icon
  names, man page and Flatpak manifest moved to `io.github._0x1_1.revivalpad`.
- Translation catalogues renamed into the `revivalpad` namespace with existing
  translations preserved and merged.
- Fixed hardcoded `#0000ff`/`#0057ae` link colours in several dialogs that were
  unreadable under dark system themes; links now follow the system palette.
- Documentation rewritten. Upstream distribution, packaging and support claims
  that do not apply to this fork were removed rather than inherited.

---

# AntiMicroX

The entries below are the upstream AntiMicroX release history, preserved as-is.
They are **not** RevivalPad releases.
