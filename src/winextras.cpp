...
static const QString PROGRAMASSOCIATIONKEY(QString("%1\\%2").arg(ROOTASSOCIATIONKEY).arg("RevivalPad.amgp"));
...
void WinExtras::writeFileAssocationToRegistry()
{
    ...
    fileAssociationReg.setValue("Default", "RevivalPad.amgp");
    fileAssociationReg.sync();

    QSettings programAssociationReg(PROGRAMASSOCIATIONKEY, QSettings::NativeFormat);
    programAssociationReg.setValue("Default", tr("RevivalPad Gamepad Profile"));
...
void WinExtras::removeFileAssociationFromRegistry()
{
    ...
    if (currentValue == "RevivalPad.amgp")
    {
 ...
 */
bool WinExtras::elevateApplication()
{
