...
MainWindow::MainWindow(QMap<SDL_JoystickID, InputDevice *> *joysticks, CommandLineUtility *cmdutility,
                       RevivalPadSettings *settings, bool graphical, QWidget *parent)
    : QMainWindow(parent)
    ...
    , trayIconMenu(nullptr)
{
    ...
    QNetworkRequest request(QUrl(QStringLiteral(REVIVALPAD_UPDATE_API_URL)));
...
void MainWindow::populateTrayIcon()
{
    ...
    QIcon icon = PadderCommon::loadIcon(REVIVALPAD_APP_ID ".trayicon", ":/images/revivalpad.png");
...
void MainWindow::restartAsElevated()
{
    ...
    msg.setText(tr("Are you sure that you want to run this program as Adminstrator?"
                   "\n\n"
                   "Some games run as Administrator which will cause events generated by " REVIVALPAD_DISPLAY_NAME " "
                   "to not be used by those games unless " REVIVALPAD_DISPLAY_NAME " is also run "
                   "as the Adminstrator. "
    ...
    {
        bool result = WinExtras::elevateApplication();
        if (result)
        {
            qApp->quit();
        } else
...
void MainWindow::updateButtonPressed()
{
    ...
    QDesktopServices::openUrl(QUrl(QStringLiteral(REVIVALPAD_LATEST_RELEASE_URL)));
}
