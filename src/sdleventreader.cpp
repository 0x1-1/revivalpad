...
SDLEventReader::SDLEventReader(QMap<SDL_JoystickID, InputDevice *> *joysticks, RevivalPadSettings *settings, QObject *parent)
    : QObject(parent)
{
    ...
    this->pollRate =
        settings->value("GamepadPollRate", GlobalVariables::RevivalPadSettings::defaultSDLGamepadPollRate).toUInt();
...
void SDLEventReader::loadSdlMappingsFromDatabase()
{
    ...
    database_file = QApplication::applicationDirPath().append("/../share/" REVIVALPAD_DATA_DIR_NAME "/gamecontrollerdb.txt");
...
RevivalPadSettings *SDLEventReader::getSettings() const { return settings; }
