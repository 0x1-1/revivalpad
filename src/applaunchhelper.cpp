...
AppLaunchHelper::AppLaunchHelper(RevivalPadSettings *settings, bool graphical, QObject *parent)
    : QObject(parent)
{
...
{
    int springScreen =
        settings->value("Mouse/SpringScreen", GlobalVariables::RevivalPadSettings::defaultSpringScreen).toInt();

    if (springScreen >= QGuiApplication::screens().count())
    {
        springScreen = -1;
        settings->setValue("Mouse/SpringScreen", GlobalVariables::RevivalPadSettings::defaultSpringScreen);
        settings->sync();
    }
...
void AppLaunchHelper::checkPointerPrecision()
{
    ...
    bool disableEnhandedPoint =
        settings->value("Mouse/DisableWinEnhancedPointer", RevivalPadSettings::defaultDisabledWinEnhanced).toBool();
...
RevivalPadSettings *AppLaunchHelper::getSettings() const { return settings; }
