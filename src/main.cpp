...
/**
 * @brief First-run import of an existing AntiMicroX (or older) configuration.
 *
 * RevivalPad keeps its own configuration namespace so it can be installed next
 * to AntiMicroX. On first run - and only if RevivalPad has no configuration of
 * its own yet - the newest available upstream configuration is *copied* into
 * the RevivalPad location, newest source first:
 *
 *   1. AntiMicroX      (antimicrox / antimicrox_settings.ini)
 *   2. antimicroX      (upstream legacy)
 *   3. antimicro       (upstream legacy)
 *
 * The original files are never moved, modified or deleted, and an existing
 * RevivalPad configuration is never overwritten.
 */
...
int main(int argc, char *argv[])
{
    ...
    QApplication revivalpad(argc, argv);
    QCoreApplication::setApplicationName(QStringLiteral(REVIVALPAD_EXECUTABLE_NAME));
    QGuiApplication::setApplicationDisplayName(QStringLiteral(REVIVALPAD_DISPLAY_NAME));
    QCoreApplication::setOrganizationName(QStringLiteral(REVIVALPAD_PRODUCT_FAMILY));
    ...
    RevivalPadSettings settings(PadderCommon::configFilePath(), QSettings::IniFormat);
    CommandLineUtility cmdutility;

    try
    {
        cmdutility.parseArguments(revivalpad);
    ...
    if (socket.state() == QLocalSocket::ConnectedState)
    {
        ...
        PRINT_STDOUT() << REVIVALPAD_DISPLAY_NAME " is already running.\n";
        ...
        QObject::connect(&revivalpad, &QApplication::aboutToQuit, joypad_worker.data(), &InputDaemon::quit);

#if QT_VERSION >= QT_VERSION_CHECK(5, 4, 0)
        QTimer::singleShot(50, &revivalpad, &QApplication::quit);
#else
        QTimer::singleShot(50, &revivalpad, SLOT(quit()));
#endif

        int result = revivalpad.exec();
    ...
    LocalRevivalPadServer *localServer = new LocalRevivalPadServer();
    ...
    revivalpad.setQuitOnLastWindowClosed(false);
    ...
    QGuiApplication::setDesktopFileName(QStringLiteral(REVIVALPAD_DESKTOP_FILE_NAME));
...
#endif
    revivalpad.installTranslator(&qtTranslator);
    ...
    QObject::connect(&revivalpad, &QApplication::aboutToQuit, localServer, &LocalRevivalPadServer::close);
    QObject::connect(&revivalpad, &QApplication::aboutToQuit, mainWindow, &MainWindow::saveAppConfig);
    QObject::connect(&revivalpad, &QApplication::aboutToQuit, mainWindow, &MainWindow::removeJoyTabs);
    QObject::connect(&revivalpad, &QApplication::aboutToQuit, &mainAppHelper, &AppLaunchHelper::revertMouseThread);
    QObject::connect(&revivalpad, &QApplication::aboutToQuit, joypad_worker.data(), &InputDaemon::quit);
    QObject::connect(&revivalpad, &QApplication::aboutToQuit, joypad_worker.data(), &InputDaemon::deleteLater);

    QObject::connect(localServer, &LocalRevivalPadServer::showHiddenWindow, mainWindow, &MainWindow::show);
    QObject::connect(localServer, &LocalRevivalPadServer::clientdisconnect, mainWindow,
    ...
    QString dbusServiceName = QStringLiteral(REVIVALPAD_DBUS_SERVICE);
    ...
    int app_result = revivalpad.exec();
