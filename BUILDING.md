...
> RevivalPad is a modified fork of AntiMicroX 3.6.1. See [FORK_NOTICE.md](./FORK_NOTICE.md).

RevivalPad is not yet packaged by any distribution, so building from source is currently the
most reliable way to get it. Release artifacts, when published, appear on this fork's own
releases page.
...
In order to build this program, open a terminal and cd into the revivalpad
directory. Enter the following commands in order to:

Clone repository

```bash
git clone https://github.com/0x1-1/revivalpad.git
```
...
Recent versions of Visual Studio (2017+) have support for cmake projects. Under Visual Studio 2022, building RevivalPad is quite straight forward.
...
- Open revivalpad as a local folder in VS22. It should pick up the `CMakeLists.txt` and offer an option to open the CMake settings editor. If it doesn't, right click on `CMakeLists.txt` in the solution explorer and select `CMake settings for revivalpad`.
...
- At this point you should be able to save your changes to regenerate the cmake cache, which will then allow you to build `revivalpad.exe` through Visual Studio.
...
cmake to build revivalpad. The following file will attempt to list some of those
...
Default: OFF. Allows for the launch of test sources with unit tests
    ...
    -DREVIVALPAD_PKG_VERSION

Default: Not defined. (feature intended for packagers) Manually define version of package displayed in info tab. When not defined building time is displayed instead. Example: `-DREVIVALPAD_PKG_VERSION=1.0.0-appimage`
...
Build revivalpad and install it in AppDir directory
...
RevivalPad is **not** published on Flathub. The manifest in this repository is for local
builds only.

```bash
flatpak install flathub org.kde.Platform//6.10 org.kde.Sdk//6.10
flatpak-builder --user --install build-flatpak/ other/io.github._0x1_1.revivalpad.yml --force-clean
```
